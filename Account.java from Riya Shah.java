...
public class Account {
    ...
    public long getBalance(String accountType) {
        return normalizeType(accountType).equals(CHECKING) ? checkingCents : savingsCents;
    }
    ...
    public void deposit(long amountCents, String accountType) {
        ...
        transactionHistory.add(new Transaction("Deposit", amountCents, type, "Deposit to " + type));
    }
    ...
    public void withdraw(long amountCents, String accountType)
        ...
        if (amountCents > available) {
            throw new InsufficientFundsException(
                "Insufficient funds in " + type + ". Available: " + Money.format(available));
        }
        if (dailyWithdrawalTotal + amountCents > DAILY_WITHDRAWAL_LIMIT_CENTS) {
            throw new DailyLimitExceededException(
                "Daily withdrawal limit of " + Money.format(DAILY_WITHDRAWAL_LIMIT_CENTS)
                + " exceeded. Remaining today: " + Money.format(getRemainingDailyLimit()));
        }
        ...
        transactionHistory.add(
            new Transaction("Withdrawal", amountCents, type, "Withdrawal from " + type));
    }
    ...
    /** For unit tests only: pretend the last withdrawal happened on another date. */
